...
def collect_txt_data(file_adress, primaries_per_spawn=2e6) -> str:
    ...
    for line in file:
        if "Spawn number:" in line:
            ...
            targetZ.append(int(line.split(" ")[4]))
            targetA.append(int(line.split(" ")[5]))
            energy_out.append(float(line.split(" ")[6]))
            energy_in.append(float(line.split(" ")[7]))

            crash_x.append(float(line.split(" ")[8]))
            crash_y.append(float(line.split(" ")[9]))
            crash_z.append(float(line.split(" ")[10]))

            region.append(int(line.split(" ")[11]))
            particle_generation.append(int(line.split(" ")[12]))
            particle_age.append(float(line.split(" ")[13]))

            source_x.append(float(line.split(" ")[14]))
            source_y.append(float(line.split(" ")[15]))
            source_z.append(float(line.split(" ")[16]))                         

    print(f"\rCollecting data: complete. {n} lines read. Time used: {round(time.time() - time_stamp, 3)} s", flush=True)
    return ncase, icode, particle_in, particle_out, \
