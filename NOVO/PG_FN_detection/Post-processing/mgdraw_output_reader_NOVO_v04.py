...
class DetectionDataStorage:
    ...
    def __init__(
            ...
            max_files: int = 100,
            lattice_usage: bool = False,
            FLUKA_to_meas_coords: bool=False
        ) -> None:
        ...
        self.lattice_usage = lattice_usage
        self.FLUKA_to_meas_coords = FLUKA_to_meas_coords
    ...
    def filter_hits(self, time_structure = "pulsed", beam_current=2, write_hits_to_file=False, g_threshold=10, n_threshold=200, hit_merging_time_limit=0.4):
        ...
        "pulsed"    :   Semi-contiuous/pulsed, and should be the most realistic. Releases bunches of 117.6 protons per 9.4 nanoseconds

        beam_current:
        Used in the conversion from FLUKA times to cyclotron times. Unit: nA
        Default: 2 [nA]

        write_hits_to_file: Flag on whether to save/override gamma/neutron/all_hits into .txt-files that can be used later
        g_threshold: Gamma energy deposition threshold [keV], default 10 keV (Deliverable 1.1)
        n_theshold: Neutron energy depositon threshold [keV], default 200 keV (Deliverable 1.1)
        '''
        ...
        for current_ncase_data in ncase_groups:
            ...
            # --------------ICODE PRE-FILTERING--------------
                ...
                for index, icode in enumerate(icodes):
                    ...
                    # Remove cross-scatter hits between lattice copies
                    if self.lattice_usage == True:
                        if raw_hit[6] == 1:
                            continue
        ...
        # self.all_hits are the combination of the rest (allows differential energy cutting)
        if len(self.neutron_hits) > 0:
            if len(self.combo_hits) > 0:
                self.all_hits = np.concatenate((self.gamma_hits, self.neutron_hits, self.combo_hits))
            else:
                self.all_hits = np.concatenate((self.gamma_hits, self.neutron_hits))
        else:
        ...
        if time_structure != None: # Only do this if FLUKA times have been rescaled. Otherwise WAY too many hits will be merged together
            ...
            new_all_hits = []
            all_hit_regions = self.all_hits[:, 12]            

            for region_number in range(1, 13):   # MREG 1 - 12

                region_mask = all_hit_regions == region_number  # Only use hits occuring in this region (mask)

                hits_in_region = self.all_hits[region_mask] # Select hits in this region
                time_order = np.argsort(hits_in_region[:, 14])  # Chronologic order of hits (FLUKA times [µs])
                hits_in_region = hits_in_region[time_order] # Order times
            ...
            # Refilling the self.gamma_hits, self.neutron_hits and self.combo_hits

            self.gamma_hits = new_all_hits[np.isin(new_all_hits[:, 1], [219, 221])]      # Selecting only gamma hits (ICODE 219 or 221)
            self.neutron_hits = new_all_hits[np.isin(new_all_hits[:, 1], [99, 100, 101, 106, 300])]  # Selecting only neutron hits (ICODE 99, 100, 101, 106 or 300)
            self.combo_hits = new_all_hits[np.isin(new_all_hits[:, 1], [500])]   # Selecting only combo hits

            # Purge new_all_hits from memory
            new_all_hits = []

        #------------END HIT MERGING BETWEEN DIFFERENT NCASES------------

        
        # Filter away hits with energy deposit lower than 10 keV (gammas) and 200 keV (neutrons)
        if len(self.gamma_hits) > 0:
            self.gamma_hits = self.gamma_hits[~(self.gamma_hits[:, 7] < round(0.001 * g_threshold, 3))]  # Remove instances where the energy deposit is lower than g_threshold (10 keV / 100 keV)
        if len(self.neutron_hits) > 0:
            self.neutron_hits = self.neutron_hits[~(self.neutron_hits[:, 7] < round(0.001 * n_threshold, 3))]  # Remove instances where the energy deposit is lower than n_threshold (200 keV)
        if len(self.combo_hits) > 0:
            self.combo_hits = self.combo_hits[~(self.combo_hits[:, 7] < round(0.001 * max(g_threshold, n_threshold), 3))]  # Remove instances where the energy deposit is lower than n_threshold (200 keV)

        # Recombine things into the all_hits list
        if len(self.neutron_hits) > 0:
            if len(self.combo_hits) > 0:
                self.all_hits = np.concatenate((self.gamma_hits, self.neutron_hits, self.combo_hits))
            else:
        ...
        print(f"Number of combo hit histories > {max(g_threshold, n_threshold)} keV: {len(self.combo_hits)}")
    ...
    def _merge_hits(self, hit_array, time_limit):
        """
        ...
        for hit_cluster in hit_clusters:
            ...
            for index, icode in enumerate(icodes):
                if icode in [217, 219, 221, 103]:
                    ...
                    # 103 is noted as a gamma because I assume you cannot know whether this electron comes from a gamma or not
            ...
            merged_hits[-1][8] = hits_2_be_merged[0, 8] # Simply use the first incident particle energy. Better than 1000 MeV
    ...
    def _event_builder(self, hit_array):
        ...
        for hit in hit_array:
            ...
            if event_ID != [hit[0], hit[15], hit[16], hit[17]]:
                ...
                # Reset event
                event = [hit]
                event_ID = [hit[0], hit[15], hit[16], hit[17]]
