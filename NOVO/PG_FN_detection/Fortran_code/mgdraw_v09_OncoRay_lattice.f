...
*------------------- Start added by Sander ----------------------------*
*    #Check if the particle enters one of the detector copies
      IF (NEWREG .GE. 34 .OR. NEWREG .LE. 37) THEN
        
*      #if so, did it arrive from the AIR region?
        IF (MREG .EQ. 38) THEN
        
*        #Increment the "enter-lattice-counter" by one
          ISPUSR(3) = ISPUSR(3) + 1
...
* 1 - 12 : Scintillator bars                                           *
* 13 - 33: PMMA target 
* 34 - 37: Lattice regions                                             *
* 38     : Air void                                                    *
* 39     : Blackbody                                                   *
* 40+    : Readout electronics and voids around scintillators          *
*
...
*
*     If the current region is the target (region 13-33)
      IF(MREG .GE. 13 .AND. MREG .LE. 33) THEN

*       Default flag of direct FN/PG is 0 (False)
        LLOUSE = 0
...
*            Flag particle to be a direct FN/PG
             LLOUSE = 1
*
...
*       Specific PG: 1H + n -> 2H + gamma-interaction (and similar)
        IF(ICODE.EQ.300 .AND. JTRACK.EQ.8 .AND. KPART(1).EQ.7) THEN
...
*      If an interaction happen outside of the target/detector, then the
*      particle is no longer considered a direct FN/PG-particle (loss of coincidence)
*
      IF (MREG .GE. 34) THEN
*
*       Reset direct FN/PG-flag to 0 (non-FN/PG)
        LLOUSE = 0
*
      END IF
...
*     If an interaction happens in the scintillator bars
      IF (MREG .GE. 1 .AND. MREG .LE. 12) THEN
*
*      If a direct FN/PG does something it shouldn't do, it loses the status of a direct FN/PG
*      This is meant to be rough check, not a full physics check.
       IF(LLOUSE .EQ. 1) THEN

*       If the interaction is not any of the "wanted" ICODEs, reset the direct FN/PG flag
...
*
*         If there are fewer or more than 2 secondaries, then it is not a wanted interaction
          IF(NP .NE. 2) THEN
            LLOUSE = 0

          ELSE
*           The only wanted interactions are JTRACK=8, KPART(1)=1, KPART(2)=8
*           Any other combination will have their direct FN/PG-flag reset to 0
            IF(
...
*        Multiply the "enter-lattice-counter" by 11 (00 = ?, 11 = One detector, OK, 22 = Cross-pollination, not OK)
         ISPUSR(4) = ISPUSR(3) * 11

*        Reassigning KPART(IP) to return something sensible (Carbon-12: -601200 -> -2 (Heavy ion))
         IF(KPART(IP) .LE. -300000) THEN

*         Write output file: 'scintillator_interactions.txt'
          WRITE(80,'(1I8,1X,6I3,1X,2F11.6,1X,3F10.5,1X,2I5,1X,5F11.6)')
     &     NCASE, ICODE, JTRACK, -2, LLOUSE, 
     &     ISPUSR(4), ISPUSR(4), Tki(IP), ETRACK-AM(JTRACK),
     &     XSCO, YSCO, ZSCO, 
     &     MREG, LTRACK, ATRACK*1E6,
     &     SPAUSR(2), SPAUSR(3), SPAUSR(4), SPAUSR(1) 
*
         ELSE
