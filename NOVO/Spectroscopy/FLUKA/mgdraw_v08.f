...
*      IF(MREG .GE. 201) THEN
      IF(MREG .GE. 202 .OR. MREG .EQ. 57) THEN
...
* 	Information on original nucleus  
*	ISPUSR(7) = ICHTAR       
*	ISPUSR(8) = IBTAR 
*       Information on production-MREG
	
	ISPUSR(6) = MREG

* 	Information on original nucleus. since 106 icode 
* 	already have a changed nucleus, check 101
* 	How to save this throught the "lifetime" of a PG/FN?
* 	Hmm denne må sjekkes. 
...
*     Denne var 57, men nå ligger range-shifter foran her. Blir da i så fall 
*     58 - 201 (ikke 57 - 200)
      IF (MREG .GE. 58 .AND. MREG .LE. 201) THEN
*
...
*      Ignore JTRACK less then -6, fix later?
       IF (JTRACK.GE.-6) THEN
       DO IP = 1, NP
...
*         Write output file: 'FN_PG_detected.txt'
          WRITE(80,'(2I9,2X,8I3,1X,2F11.6,1X,3F10.5,1X,2I5,1X,4F11.6)')
     &     ISPUSR(6),
     &     NCASE, ICODE, JTRACK, -2, LLOUSE, 
     &     ICHTAR, IBTAR, ISPUSR(7), ISPUSR(8),
     &     Tki(IP), ETRACK-AM(JTRACK),
     &     XSCO, YSCO, ZSCO, 
